...
import { updatePath } from "@/app/actions/admin-paths";

export default async function AdminEditPathPage({
  params,
}: {
  ...
  async function submit(formData: FormData) {
    "use server";
    const result = await updatePath(slug, {
  ...
  return (
    <div>
      ...
      <h1 className="mt-4 text-2xl font-light text-neutral-800 dark:text-neutral-100">
        Edit path: {path.title}
      </h1>
      <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
        Slug (URL) cannot be changed: /{path.slug}
      </p>
      <form action={submit} className="mt-6 max-w-xl space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            name="introduction"
            rows={4}
            defaultValue={path.introduction}
            className="mt-1 w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-100"
          />
        </div>
        <div>
