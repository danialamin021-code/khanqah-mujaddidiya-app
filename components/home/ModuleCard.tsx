...
export default function ModuleCard({
  ...
  return (
    <Link
      href={`/modules/${slug}`}
      className="group flex min-h-[100px] items-stretch gap-0 overflow-hidden rounded-2xl border border-green-soft bg-light-green/50 shadow-sm transition-all duration-200 hover:scale-[1.01] hover:border-deep-green/20 hover:bg-light-green hover:shadow-md"
    >
      {/* Left: icon + module name */}
      <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-2 p-4 text-center">
        <span
        ...
        <span className="font-heading text-sm font-normal text-deep-green leading-tight">
          {name}
        </span>
      </div>
      {/* Right: clear module image (fixed aspect for consistent layout) */}
      <div className="relative w-24 shrink-0 sm:w-28" style={{ aspectRatio: "1" }}>
        <Image
          src={imageSrc}
          alt=""
          fill
